...
module CalendarHelper
  ...
  def calendar(options = {}, &block)
    ...
    defaults = {
      ...
      :week_number_link1   => nil,
      :week_number_link2   => nil,
      :show_month          => 1,
      :months              => nil,
    }
    ...
    cal << %(<td class="#{options[:week_number_class]}"><a href="#{options[:week_number_link1]}&week_number=#{week_number(begin_of_week, options[:week_number_format])}">#{week_number(begin_of_week, options[:week_number_format])}</a></td>) if options[:show_week_numbers]
    ...
    first.upto(last) do |cur|
      ...
      if cur.wday == last_weekday
        ...
        if cur != last
          cal << %(<tr>)
          cal << %(<td class="#{options[:week_number_class]}"><a href="#{options[:week_number_link1]}&week_number=#{week_number(cur + 1, options[:week_number_format])}">#{week_number(cur + 1, options[:week_number_format])}</a></td>) if options[:show_week_numbers]
        end
      end
    end
  ...
  def generate_cell(cell_text, cell_attrs)
    ...
    "<td #{cell_attrs}><a href=#{request.fullpath}&day=#{cell_text}>#{cell_text}</a></td>"
  end
